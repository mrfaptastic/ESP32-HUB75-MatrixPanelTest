...
#ifndef MATRIX_WIDTH
	#define MATRIX_WIDTH                64*1 // width * chain length
#endif
...
class RGB64x32MatrixPanel_I2S_DMA : public Adafruit_GFX {	
  ...
  public:
    ...
    // Default signal timing and ordering variables...
    int16_t oe1_start_x_pos = 0;
    int16_t oe1_end_x_pos   = 0; // length i pixels

    int16_t lat_start_x_pos   = PIXELS_PER_ROW-1;
    int16_t lat_end_x_pos     = PIXELS_PER_ROW-1;

    int16_t oe2_start_x_pos  = PIXELS_PER_ROW-2;
    int16_t oe2_end_x_pos    = PIXELS_PER_ROW-2; // length i pixels  
